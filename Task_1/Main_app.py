...
class MainApp(QMainWindow, ui):
    def __init__(self):
        ...
        self.export_1_btn.clicked.connect(lambda: self.export_graph_to_pdf(1))
        self.export_2_btn = self.findChild(QPushButton, 'export_2_btn')
        self.export_2_btn.clicked.connect(lambda: self.export_graph_to_pdf(2))
    ...
    def reset_view(self, channel):
        if self.link_channels_checkbox.isChecked():
            self.reset_view_both()
        else:
    ...
    def scroll_graph_horizontal(self, value, channel):
            if self.paused[channel - 1]:
                ...
                if value == 1000 and max_display_range > self.x_end:
                    self.enable_x_axis_auto_pan(channel)
    ...
    def update_speed(self, value, channel):
        if self.link_channels_checkbox.isChecked():
            self.update_speed_for_both(value,channel)
        else:
            self.speed_slider_1.setEnabled(True)
            min_speed = 1
            max_speed = 10
    ...
    def update_speed_for_both(self, value, channel):
        channel = 1
        self.speed_slider_1.setEnabled(True)
        min_speed = 1
        max_speed = 10
    ...
    def screenshot_graph(self, channel, image_path):
        pixmap = self.graphs[channel - 1].grab()
        pixmap.save(image_path)

    def export_graph_to_pdf(self, channel):
        # Take a screenshot of the graph and save it as an image
        img_path = f'./screen_shots/channel_{channel}_graph.png'
        self.screenshot_graph(channel, img_path)

        # Show a file dialog to choose the save location
        options = QFileDialog.Options()
        file_path, _ = QFileDialog.getSaveFileName(self, f'Save PDF for Channel {channel}', "", "PDF Files (*.pdf)",
                                                   options=options)
        if file_path:
            # Create a PDF document
            c = canvas.Canvas(file_path, pagesize=letter)
            # Add a title at the top of the page centered horizontally with a blue font color
            title = f'Channel_{channel} Report'
            ...
            stats_df = self.generate_statistics_table(channel)
